...
{
    public partial class SistemaPage : System.Web.UI.MasterPage
    {
        ...
        private bool AgregarItemsDeModulo(HtmlGenericControl nodoPadre, List<E.VMItemModulo> modulosNodo, int nivel)
        {
            ...
            foreach (E.VMItemModulo item in modulosNodo)
            {
                ...
                if ((nivel == 0 && item.IdItemPadre == 0) || (nivel > 0))
                {
                    ...
                    link.Attributes.Add("onclick", "navigateTo(this)");
                    link.Attributes.Add("data-target", item.Url+"?tkn="+HttpUtility.UrlPathEncode(sistema.TOKEN_SESION) );
